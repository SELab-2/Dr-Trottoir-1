<template>
  ...
  >
    ...
    <template v-slot:append>
      <v-chip label color="blue" class="ml-3 align-top" v-if='building.data.length === 1'>
        <v-icon icon="mdi-calendar-clock"></v-icon>
        <p class="ml-2">{{ building.data[0].date }}</p>
      </v-chip>
      <v-icon @click="expanded = !expanded" :icon="expanded ? 'mdi-menu-up' : 'mdi-menu-down'" class='dropdown-button' v-else/>
    </template>
  </v-card>
  <div v-if='building.data.length > 1 && expanded'>
    <div v-for='(datum, id) of building.data' :key='id' class='w-100 px-2 pt-1'>
