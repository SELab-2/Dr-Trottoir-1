<template>
  <CardLayout class="pa-8 pb-0 space-y">
    <div
      ...
      "
    >
      <div @click="router.push({ name: 'building_id', params: { id: building.id }, })">
        <h2>{{ building.name }}</h2>
        <p>
      ...
      <RoundedButton
        ...
        :value="progress.arrival ? new Date(progress.arrival).toLocaleTimeString() : 'Start'"
        @click="() => start()"
      ></RoundedButton>
      <RoundedButton
        v-if="progress"
        icon="mdi-clock-end"
        :value="progress.departure ? new Date(progress.departure).toLocaleTimeString() : 'Einde'"
        @click="() => end()"
      ></RoundedButton>
    ...
    <div>
      ...
      <div
        class="carousel"
        v-if="progress?.images.length ?? 0 > 0"
      >
        <div
          class="carousel-item"
