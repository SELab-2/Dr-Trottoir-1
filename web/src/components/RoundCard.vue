<template>
  ...
  >
    <v-progress-linear
      ...
      :color="progress === 0 ? 'error' : progress === 100 ? 'success' : 'warning' "
    />
    <!-- Set round name as title -->
    <template v-slot:title>
      {{ round_name }}
      <v-icon end v-if="round_comments" class="">mdi-comment-alert-outline</v-icon>
    </template>
    ...
    <template v-slot:append>
      <v-chip
        ...
        :color="progress === 0 ? 'error' : progress === 100 ? 'success' : 'warning'"
      >
      <v-icon :icon="progress === 0 ? 'mdi-close' : progress === 100 ? 'mdi-check' : 'mdi-progress-clock'"></v-icon>
      {{ progress === 0 ? 'Niet begonnen' : progress === 100 ? 'Klaar' : 'Bezig '+building_index+'/'+total_buildings }}
      </v-chip>
    </template>
    <v-chip
      label
